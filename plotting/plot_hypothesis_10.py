...
sns.boxplot(data=recalls[recalls['algo'].isin(['lsh', 'hnsw'])], x='algo', y='Recall', hue='instance', legend=False, ax=ax, showfliers=False)
...
colors = list(algo_to_color.values())
for patch_idx, patch in enumerate(ax.patches):
    patch.set_facecolor(colors[patch_idx % (len(['lsh', 'hnsw']))])

plt.savefig(plot_path + "_recalls.pdf")

plt.clf()

sns.lineplot(data=recalls[recalls['algo'].isin(['lsh', 'hnsw'])], x='k', y='Recall', hue='algo', palette=algo_to_color, legend=False)
...
sns.lineplot(data=recalls[recalls['algo'].isin(['lsh'])], x='instance', y='Recall', hue='k', legend=False)

plt.savefig(plot_path + "_recalls_lsh_only.pdf")
