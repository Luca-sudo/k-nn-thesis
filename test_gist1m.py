...
def create_index(ds, sites, n_dims, n_planes, i):
...
for ds in ds_to_test:
    print(f'Creating ds: {to_string(ds)}')
    (index, rand_sample) = create_index(ds, sites, n_dims[0], n_planes[0], 0)
    var_value = file.attrs['var_values'][0]

    for i in range(sample_size // 20):
        print(f'\tChunk {i} of 50')
