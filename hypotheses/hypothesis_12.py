...
target_lids = [20, 30, 40, 50, 60, 70, 80, 90, 100]
base_lid = 5
base_lid_center = [0 for _ in range(100)]
base_lid_center[0] = 1.0
deviation_magnitude = 0.5
n_sites = [100000 for _ in range(len(target_lids))]
n_dims = [100 for i in n_sites]
n_planes = [dim * 2 for dim in n_dims]
max_k = 100
sample_size = 30
filepath = "data/hypothesis_12.h5"
def site_generator(i):
    sites = np.asarray(list(map(lambda x: np.pad(x, (0, 100 - base_lid), 'constant'), uniform_direction(base_lid).rvs(n_sites[i]))))
    target_deviation = deviation_magnitude / np.sqrt(n_dims[i])
    noise = np.random.normal(0, target_deviation, (n_sites[i], target_lids[i]))
    for j in range(len(sites)):
        cosine_sim = spatial.cosine(base_lid_center, sites[j]) / 2.0
        additional_lid = np.floor(cosine_sim * target_lids[i])
        site_noise = noise[j][:additional_lid]
        site_noise[additional_lid:] = [0.0] * (target_lids[i] - additional_lid)
        sites[j][base_lid:target_lid] = site_noise
    return sites
