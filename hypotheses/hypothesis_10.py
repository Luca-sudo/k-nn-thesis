...
lids = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]
n_sites = [100000 for _ in lids]
n_dims = [1000 for i in n_sites]
n_planes = [dim * 2 for dim in n_dims]
max_k = 100
sample_size = 30
filepath = "data/hypothesis_10.h5"
site_generator = lambda i: np.asarray(list(map(lambda x: np.pad(x, (0, 1000 - lids[i]), 'constant'), uniform_direction(lids[i]).rvs(n_sites[i]))))
query_generator = lambda i: np.asarray(list(map(lambda x: np.pad(x, (0, 1000 - lids[i]), 'constant'), uniform_direction(lids[i]).rvs(sample_size))))


hypothesis = "Local intrinsic dimensionality affects LSH more severely in extreme dimensions than HNSW."

description = """
Successive instances sample points from spheres of increasing dimensions, which are embedded into the 1000-dimensional feature space.
"""
