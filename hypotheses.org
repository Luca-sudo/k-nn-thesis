...
#+begin_src python :tangle hypotheses/hypothesis_3.py
    n_sites = extents[i] ** n_dims
    print(f'Generating instance {i}:')
    time_start = time.perf_counter()
    sites = [(np.float64(x),np.float64(y)) for x in range(extents[i]) for y in range(extents[i])]
...
#+begin_src python :tangle hypotheses/hypothesis_3.py
    queries = np.random.uniform(0, extents[i] - 1.0, (sample_size, n_dims))
    time_start = time.perf_counter()
    index = faiss.IndexFlatL2(n_dims)
    index.add(sites)
    time_end = time.perf_counter()
    ...
    for sample_idx in range(sample_size):
        temp = [0.0 for i in range(n_sites)]
